  ...
  function getBankBin(cardNo) {
    ...
    if (isNaN(cardNo)) {
      ...
      if (isNaN(cardNo)) {
        errMsg.error_msg = cardNo + ':The card number must be numbers';
        return errMsg;
      }
    }
    if (cardNo.toString().length < 15 || cardNo.toString().length > 19) {
      errMsg.error_msg = cardNo + ':The input card number must more than 15 digit and less than 19 digit.';
      return errMsg;
    }
